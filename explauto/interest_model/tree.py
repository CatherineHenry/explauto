...
class InterestTree(InterestModel, Observable):
    ...
    def random_walk_region_deletion(self, tree, pathing=None, probability_of_region_deletion=0.3):
        ...
        elif self.interest_tree_rng.random() < probability_of_region_deletion: # automatically increase likelihood of  as we get deeper into tree bc Bernoulli Process
            tree.leafnode = True
            tree.lower = None
            tree.greater = None
    ...
    def update(self, xy, ms, flow_uuid=None):
        ...
        if self.competence_measure is prediction_error_cos_dist_exp:
            ...
            self.emit(f"[{flow_uuid}] competence", f"[cos sim: {round(cos_sim, 4)}, cos dist: {round(cos_dist, 4)}] bounded cos distance between target and reached: {competence}")
        elif  self.competence_measure is competence_cos_dist_exp:
            cos_sim, cos_dist, competence = self.competence_measure(xy, ms, self.bounds)
            self.emit(f"[{flow_uuid}] competence", f"[cos sim: {round(cos_sim, 4)}, cos dist: {round(cos_dist, 4)}], competence {competence}")
...
class Tree(Observable):
    ...
    def progress_idxs(self, idxs):
        ...
        """
        if self.progress_measure == "none": # If we aren't splitting, don't bother calculating learning potential
            return 0
...
interest_models = {'tree': (InterestTree, {'default': {'max_points_per_region': 100,
                                           ...
                                           'cozmo_clip': {'max_points_per_region': 30, # twenty seems good so far
                                                          ...
                                                          'max_turn_counts': [30, 10], # 1st execution, all future executions
                                                          'region_deletion_alphas':None},
                                           ...
                                           'cozmo_clip_cos_sim_split_progressive_splits_epsilon_greedy_sampling_new_learning_potential_calculation_smaller_initial_execution': {
                                               ...
                                               'sampling_mode': {'mode':'epsilon_greedy',
                                                                 ...
                                                                 'volume':False},
                                               'plot_objects': None,
                                               'region_deletion_alphas':[(0.3, 0.2), (0.2, 0.15), (0.2, 0.15), (0.1, 0.1)],
                                               'max_turn_counts': [45, 20, 20, 20], # 1st execution, all future executions
                                               'progressive_split_ranges': {'max_ppr': [(8, 15), (15,20), (15,20), (20,20)],
                                                                            'prog_win': [(8, 8)]},
                                           },
