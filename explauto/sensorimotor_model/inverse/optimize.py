...
class OptimizedInverseModel(inverse.InverseModel):
    ...
    def __init__(self, dim_x=None, dim_y=None, fwd_model=None, constraints=(), **kwargs):
        """Construst an inverse model from a dimensions and constraints set
        Default to a LWR model for the forward model.
        """      
        if fwd_model:
            self.dim_x = fwd_model.dim_x
            self.dim_y = fwd_model.dim_y
            self.fwd_model = fwd_model
        else:
            self.dim_x = dim_x
            self.dim_y = dim_y
            self.fwd_model = LWLRForwardModel(self.dim_x, self.dim_y, **kwargs)
    ...
    def _setuplimits(self, constraints):
        ...
        assert len(self.constraints) == self.fwd_model.dim_x
        self.goal = None
    ...
    def _error(self, x):
        ...
        """
        y_pred = self.fwd_model.predict_y(x)
    ...
    def _error_dm(self, m, dm, s):
        ...
        """
        pred = self.fwd_model.predict_given_context(np.hstack((m, dm)), s, list(range(len(s))))
